...
#pragma HLS ARRAY_PARTITION variable=biases complete
 ...
 VITIS_LOOP_22_1: for (int i = 0; i < N; i += S) {
        VITIS_LOOP_23_2: for (int k = 0; k < K; k++) {
            int sum = 0;
            VITIS_LOOP_25_3: for (int j = 0; j < F; j++) {
                int d = (j - P) * D;
                if (i + d >= 0 && i + d < N) {
...
void avg_pool1d(
    ...
    VITIS_LOOP_54_1: for (int i = 0; i < N; i += stride) {
        sum += input.read();
        count++;
        if (count == size) {
...
#pragma HLS ARRAY_PARTITION variable=biases complete
 ...
 VITIS_LOOP_76_1: for (int i = 0; i < K; i++) {
        int sum = 0;
        VITIS_LOOP_78_2: for (int j = 0; j < K; j++) {
...
__attribute__((sdx_kernel("inference", 0))) void inference(
    hls::stream< ap_int<16> > &input,
    hls::stream< ap_int<16> > &output,
...
#line 13 "/home/saverio/Documents/x-heep-app/heep_ultrasound/src/hls/ultrasound_accelerator/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=inference
# 95 "HLS_CNN.cpp"
